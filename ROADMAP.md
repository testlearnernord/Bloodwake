# Roadmap

## Milestones

- 0.5 completed
- 0.6.1 Population Foundation ✅
- 0.6.2 Blood & Combat Feeding ✅
  - 0.6.2a Unified Vampire Vitae ✅
  - 0.6.2b Resonance Feed / Drain choices ✅
  - 0.6.2c Combat Feeding ✅
  - 0.6.2d Combat Feeding presentation / Clergy death hotfix ✅
  - 0.6.2e Circular Predatory Bite timing ✅
- 0.6.3 Human Recruitment and Housing (current)
  - 0.6.3a Human Thralls: Enthrallment, Control, Housing & Food ✅
  - 0.6.3a1 Thrall playtest fixes ✅
  - 0.6.3b Human Work Foundation ✅
  - 0.6.3b1 Thrall Elevation ✅
  - 0.6.3b2 Domain World Presence ✅
  - 0.6.3b3 Visible Work & Actor Task Foundation ✅
  - 0.6.3b4 Simulation Architecture Audit & Transition Plan ✅
  - 0.6.3c Nightly World Variation & escaped-human lifecycle ✅
  - 0.6.3c1 Daylight shelter & guard-slot stabilization ✅
  - 0.6.3d Blood Stock & Blood Cellar Foundation (current)
  - 0.6.3e Stabilization & Legacy Purge
- 0.6.4 Dominion, Torpor, Vampire Politics & Orders
- 0.6.5 Continuous World Simulation
  - 0.6.5a World Clock & solar cycle
  - 0.6.5b Authoritative actor tasks & reservations
  - 0.6.5c Physical containers, carrying & hauling
  - 0.6.5d Gathering, hunting & world-resource conversion
  - 0.6.5e Crafting & construction conversion
  - 0.6.5f Upkeep, Blood Donors, rest & timed population consequences
  - 0.6.5g Continuous Simulation Legacy Purge
- 0.7 Economy, Facilities, Skills & Training
- 0.8 Awakening Tutorial / Story and Region 1 progression
- 0.9 First Regional Boss and region unlock loop
- 1.0 Threat, Stronghold Raids and Defense
- 1.1 Character Genetics and Bloodline Foundations

See `docs/SIMULATION_TRANSITION_PLAN.md` for the authority audit, anti-fake-simulation rules, replacement order, and legacy deletion register for 0.6.5.
