...
describe('save serialization and validation', () => {
  ...
  it('serializes and reloads a save slot (v10)', async () => {
  ...
  it('exports and imports a valid v10 save payload', () => {
  ...
  it('rejects v10 saves with non-finite player vitae values', () => {
  ...
  it('rejects v8 saves with a clear incompatibility error', () => {
    const base = createNewGameState({ seed: 'old-v8' });
    const v8Like = { ...base, version: 8 };
    expect(() => migrateSaveGame(v8Like)).toThrow(/incompatible older game version/);
  });

  it('rejects v9 saves with a clear incompatibility error', () => {
    const base = createNewGameState({ seed: 'old-v9' });
    const v9Like = { ...base, version: 9 };
  ...
  it('rejects malformed inventory entries in v10 saves', () => {
  ...
  it('preserves turned vampire vassals across v10 save export and reload', () => {
  ...
  it('rejects v10 saves that still contain a legacy hunger field with a targeted error', () => {
  ...
  it('rejects v10 saves with malformed humanServants records', () => {
  ...
  it('rejects v10 saves with malformed vampireVassals records', () => {
    const state = createNewGameState({ seed: 'malformed-vv' });
    const bad = { ...state, vampireVassals: [{ id: 'x', kind: 'wrong_kind' }] };
    expect(validateSaveGame(bad)).toBe(false);
  });

  it('rejects v10 saves with duplicate IDs within humanServants', () => {
  ...
  it('rejects v10 saves with duplicate IDs within vampireVassals', () => {
  ...
  it('rejects v10 saves with a shared ID across both population collections', () => {
  ...
  it('rejects malformed v10 Blood Resonance metadata', () => {
  ...
  it('rejects malformed v10 Resolve metadata', () => {
  ...
  it('rejects stale v10 free-human metadata fields', () => {
