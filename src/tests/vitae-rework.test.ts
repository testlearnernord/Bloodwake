...
describe('unified vampire Vitae model', () => {
  ...
  it('uses save v10 and still rejects older or stale hunger fields', () => {
    const state = createNewGameState({ seed: 'save-v10' });
    expect(SAVE_FORMAT_VERSION).toBe(10);
    expect(migrateSaveGame(state).version).toBe(10);
